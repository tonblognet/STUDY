...
import { normalizeUserState, userStateSchema } from "@/lib/user-state/validation";
...
export async function GET() {
  ...
  try {
    return NextResponse.json({ state: await loadUserState(user), storage: "account" });
  } catch (error) {
...
export async function PUT(request: Request) {
  ...
  if (contentLength > 64_000) return NextResponse.json({ error: "payload_too_large" }, { status: 413 });
  ...
  try {
    ...
    return NextResponse.json({ state: await saveUserState(user, state), storage: "account" });
  } catch (error) {
