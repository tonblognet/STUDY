...
export function DataSourceLink<T>({
  field,
  compact = false,
}: {
  ...
  return (
    <span className="data-source-line">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label={`Открыть официальный источник: ${field.sourceName}`}
      >
        {compact ? "Источник" : field.sourceName} ↗
      </a>
      {!compact && (
        <time dateTime={field.checkedAt}>
          проверено{" "}
          {new Intl.DateTimeFormat("ru-RU").format(new Date(field.checkedAt))}
        </time>
      )}
    </span>
  );
...
export function SourcedMetric<T>({
  label,
  field,
  format = String,
}: {
  ...
  return (
    <div className={`sourced-metric sourced-metric-${field.status}`}>
      <span className="metric-label">{label}</span>
      <strong>
        {field.value === null ? "Нет данных" : format(field.value)}
      </strong>
...
export function CompletenessBadge({
  ...
  return (
    <div
      ...
      aria-label={`Полнота карточки ${percent} процентов`}
    >
      <span>
        <b>{percent}%</b> полноты
      </span>
      ...
      {missing.length > 0 && (
        <small>
          Не хватает: {missing.slice(0, 3).join(", ")}
          {missing.length > 3 ? "…" : ""}
        </small>
      )}
    </div>
