...
const usable = <T>(field: SourcedValue<T>) =>
  field.status === "verified" || field.status === "not_applicable";
...
export function calculateCompleteness(program: Program): CompletenessResult {
  const checks: Array<[string, number, boolean]> = [
    ...
    [
      "история",
      6,
      program.passingHistory.length +
        program.placesHistory.length +
        program.tuitionHistory.length >=
        3,
    ],
  ];
  const total = checks.reduce((sum, [, weight]) => sum + weight, 0);
  const earned = checks.reduce(
    (sum, [, weight, complete]) => sum + (complete ? weight : 0),
    0,
  );
  return {
