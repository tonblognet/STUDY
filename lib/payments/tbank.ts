...
export function verifyTbankToken(payload: TbankPayload, password: string): boolean {
  const received = typeof payload.Token === "string" ? payload.Token.toLowerCase() : "";
  const expected = createTbankToken(payload, password);
  if (received.length !== expected.length) return false;
  return timingSafeEqual(Buffer.from(received, "utf8"), Buffer.from(expected, "utf8"));
}

export async function initTbankPayment(payload: TbankPayload, config: { password: string; apiUrl?: string }): Promise<TbankInitResponse> {
  const body = { ...payload, Token: createTbankToken(payload, config.password) };
  const response = await fetch(`${config.apiUrl ?? "https://securepay.tinkoff.ru/v2"}/Init`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(body),
    signal: AbortSignal.timeout(10_000),
  });
  if (!response.ok) throw new Error(`T-Bank Init returned HTTP ${response.status}`);
  const result = await response.json() as TbankInitResponse;
  if (!result.Success || !result.PaymentURL || !result.PaymentId) {
    throw new Error(result.Details || result.Message || `T-Bank Init failed with ${result.ErrorCode}`);
  }
  return result;
}
