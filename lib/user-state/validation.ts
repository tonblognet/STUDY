...
const scoreSetSchema = z.object({
  id: z.string().trim().min(1).max(80).regex(/^[\p{L}\p{N}_-]+$/u),
  name: z.string().trim().min(1).max(80),
  scores: z.record(z.string().trim().min(1).max(80), z.number().int().min(0).max(100)),
...
export function normalizeUserState(
  ...
  return {
    ...
    scoreSets: [...new Map(state.scoreSets.map((item) => [item.id, item])).values()]
